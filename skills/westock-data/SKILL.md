---
name: westock-data
description: 查询A股、港股、美股个股/指数/ETF的详细数据，包括：实时行情、K线/分时、财务报表（三大报表多期查询，支持跨市场批量对比）、资金流向、技术指标、筹码分析、机构评级/研报/一致预期、新闻公告、股东结构、分红除权、业绩预告、公司简况、ETF基金数据（详情/持仓/净值）；以及大盘指数、行业/板块行情、热搜、股单、新股日历、投资日历、脱水研报等市场数据。注意：本工具"查已知股票/指数的数据"；按条件选股（如"PE<20的股票"）请用
  westock-tool；**查询用户自选股列表请用 westock-portfolio**
